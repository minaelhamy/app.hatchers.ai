    ...
    <div class="verify-scene">
        <div class="verify-frame">
            <section class="verify-card">
                ...
                <form method="POST" action="{{ route('verification.login.verify') }}" class="verify-form">
                    @csrf
                    <label>
                        <div class="verify-label">Sign-In Code</div>
                    ...
                    <button class="verify-submit" type="submit">Verify and enter dashboard</button>
                </form>

                <form method="POST" action="{{ route('verification.login.resend') }}" style="margin-top: 14px; text-align: left;">
                    @csrf
                    <button class="verify-secondary" type="submit">Resend sign-in code</button>
                </form>

                <p class="verify-footnote">
                    Need to restart? <a href="{{ route('login') }}">Back to login</a>
                </p>
            </section>
        </div>
    </div>
