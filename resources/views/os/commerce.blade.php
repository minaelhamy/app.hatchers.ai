...
@section('head')
    <style>
        ...
        .commerce-embed { padding:20px 22px 24px; background:var(--surface); min-height:100%; }
...
@section('content')
    @php ob_start(); @endphp
    <div class="{{ $osEmbedMode ? 'commerce-embed' : 'workspace' }}">
        <div class="panel">
            ...
            <div class="panel-body">
                ...
                <div class="tab-row">
                    <a class="tab {{ $commerceView === 'overview' ? 'active' : '' }}" href="{{ route('founder.commerce', array_filter(['view' => 'overview', 'os_embed' => $osEmbedMode ? 1 : null])) }}">Overview</a>
                    <a class="tab {{ $commerceView === 'offers' ? 'active' : '' }}" href="{{ route('founder.commerce', array_filter(['view' => 'offers', 'os_embed' => $osEmbedMode ? 1 : null])) }}">Offers</a>
                    <a class="tab {{ $commerceView === 'operations' ? 'active' : '' }}" href="{{ route('founder.commerce', array_filter(['view' => 'operations', 'os_embed' => $osEmbedMode ? 1 : null])) }}">Operations</a>
                    <a class="tab {{ $commerceView === 'money' ? 'active' : '' }}" href="{{ route('founder.commerce', array_filter(['view' => 'money', 'os_embed' => $osEmbedMode ? 1 : null])) }}">Money</a>
                </div>
                ...
                @elseif ($commerceView === 'offers')
                    <section class="info-grid">
                        <div class="info-card"><strong>Offer system</strong><p class="muted" style="margin-top:8px;">Use this area to shape what you sell, how it is priced, and how it appears in the storefront.</p><a href="{{ route('website', array_filter(['os_embed' => $osEmbedMode ? 1 : null])) }}" class="tab" style="margin-top:12px;">Open website workspace</a></div>
    ...
    @php $commerceContent = ob_get_clean(); @endphp
